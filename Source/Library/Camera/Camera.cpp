...
namespace library
{
    ...
    void Camera::HandleInput(_In_ const DirectionsInput& directions, _In_ const MouseRelativeMovement& mouseRelativeMovement, _In_ FLOAT deltaTime)
    {
        ...
        {
            XMVECTOR xyzMove = XMVectorSet(xMove, yMove, zMove, 0.0f);
            xyzMove = XMVector3Normalize(xyzMove);
            xyzMove *= m_travelSpeed * deltaTime;
            XMFLOAT3 vMove;
            XMStoreFloat3(&vMove, xyzMove);
        ...
        if (yawRotation != 0 || pitchRotation != 0) 
        {
            ...
            //Because We only use yaw and pitch, I do Vector2Normalize
            xyRotation = XMVector2Normalize(xyRotation);
            xyRotation *= m_rotationSpeed * deltaTime;

            XMFLOAT2  vRotation;
            XMStoreFloat2(&vRotation, xyRotation);
            m_yaw += vRotation.x;
            m_pitch += vRotation.y;
    ...
    HRESULT Camera::Initialize(_In_ ID3D11Device* device)
    {
        ...
        XMStoreFloat4(&camPosition, m_eye);
        CBChangeOnCameraMovement cbCameraMovement = {
            .View = m_view,
            .CameraPosition = camPosition
        };
        
        D3D11_SUBRESOURCE_DATA cInitData = {
            .pSysMem = &cbCameraMovement,
            .SysMemPitch = 0,
            .SysMemSlicePitch = 0
        };

        HRESULT hr;
        hr = device->CreateBuffer(&cBufferDesc, &cInitData, m_cbChangeOnCameraMovement.GetAddressOf());
    ...
    void Camera::Update(_In_ FLOAT deltaTime)
    {
        ...
        XMMATRIX yawRotation = XMMatrixRotationY(m_yaw);
