...
namespace library
{
    ...
    void Renderable::calculateNormalMapVectors()
    {
        ...
        const SimpleVertex* vertices = getVertices();
        ...
        for (UINT i = 0u; i < uNumFaces; i++) {
            XMFLOAT3 tangent = {};
            XMFLOAT3 bitangent = {};
            calculateTangentBitangent(vertices[aIndices[i * 3]], vertices[aIndices[i * 3 + 1]], vertices[aIndices[i * 3 + 2]], tangent, bitangent);
