...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    if (FAILED(mainScene->AddRenderable(L"Wall", wall)))
        return 0;
    if (FAILED(mainScene->SetVertexShaderOfRenderable(L"Wall", L"EnvironmentMapShader")))
        return 0;
    if (FAILED(mainScene->SetPixelShaderOfRenderable(L"Wall", L"EnvironmentMapShader")))
        return 0;
